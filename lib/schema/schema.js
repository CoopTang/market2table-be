...
const ProductType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    vendor: {
      ...
      resolve(parent, args) {
        return database('vendors')
          .where('vendors.id', parent.vendor_id).first()
      }
    }
  })
});
...
const RootQuery = new GraphQLObjectType({
  ...
  fields: {
    ...
    product: {
      ...
      args: { id: { type: GraphQLID }},
      resolve(parent, args) {
        return database('products').where('id', args.id).first()
      }
    },
    products: {
...
const Mutation = new GraphQLObjectType({
  ...
  fields: {
    ...
    updateVendor: {
      type: VendorType,
      args: { //these are the arguments that we need
      ...
      resolve(parent, args) { //resolver
       const vendor = find(vendors, { id: args.id })
       if(!vendor) {
